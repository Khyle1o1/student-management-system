-- ========================================
-- MATERIALIZED VIEWS FOR PERFORMANCE (FIXED)
-- ========================================
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_mv_student_stats_college 
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_mv_student_stats_course 
  ON mv_student_stats_by_course(college, course);

CREATE INDEX IF NOT EXISTS idx_mv_student_stats_course_college 
...
SELECT 
  ...
  COUNT(DISTINCT a.student_id) FILTER (
    ...
    AND (COALESCE(e.attendance_type, 'IN_ONLY') = 'IN_ONLY' OR (e.attendance_type = 'IN_OUT' AND a.time_out IS NOT NULL))
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_mv_event_stats_id ON mv_event_stats(event_id);
CREATE INDEX IF NOT EXISTS idx_mv_event_stats_date ON mv_event_stats(date DESC);
CREATE INDEX IF NOT EXISTS idx_mv_event_stats_scope ON mv_event_stats(scope_type, scope_college, scope_course);
CREATE INDEX IF NOT EXISTS idx_mv_event_stats_status ON mv_event_stats(status);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_mv_payment_stats_fee ON mv_payment_stats_by_fee(fee_id);
CREATE INDEX IF NOT EXISTS idx_mv_payment_stats_scope ON mv_payment_stats_by_fee(scope_type, scope_college);
CREATE INDEX IF NOT EXISTS idx_mv_payment_stats_active ON mv_payment_stats_by_fee(is_active);
CREATE INDEX IF NOT EXISTS idx_mv_payment_stats_due_date ON mv_payment_stats_by_fee(due_date);
...
SELECT 
  ...
  (SELECT COUNT(*) FROM events WHERE date = CURRENT_DATE) as events_today,
  ...
  -- Payment metrics
  (SELECT COUNT(*) FROM payments WHERE status = 'UNPAID') as total_unpaid_payments,
  (SELECT COUNT(*) FROM payments WHERE status = 'PENDING') as total_pending_payments,
  (SELECT COUNT(*) FROM payments WHERE status = 'PAID') as total_paid_payments,
  ...
  (SELECT SUM(amount) FROM payments WHERE status = 'UNPAID') as total_outstanding,
  ...
  -- Feedback metrics (if table exists)
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_mv_attendance_rate_event ON mv_attendance_rate_by_event(event_id);
CREATE INDEX IF NOT EXISTS idx_mv_attendance_rate_date ON mv_attendance_rate_by_event(date DESC);
...
BEGIN
  ...
  RAISE NOTICE '================================================';
  RAISE NOTICE 'Created 6 materialized views:';
  RAISE NOTICE '  1. mv_student_stats_by_college';
  RAISE NOTICE '  2. mv_student_stats_by_course';
  RAISE NOTICE '  3. mv_event_stats';
  RAISE NOTICE '  4. mv_payment_stats_by_fee';
  RAISE NOTICE '  5. mv_daily_dashboard_summary';
  RAISE NOTICE '  6. mv_attendance_rate_by_event';
  RAISE NOTICE '';
  RAISE NOTICE 'Set up a cron job to refresh views periodically.';
  RAISE NOTICE 'Recommended: Every 5-15 minutes';
  RAISE NOTICE '';
  RAISE NOTICE 'Manual refresh:';
  RAISE NOTICE '  SELECT * FROM refresh_performance_materialized_views();';
  RAISE NOTICE '================================================';
END $$;
