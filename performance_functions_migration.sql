...
-- Example usage:
-- SELECT * FROM assign_fee_to_students(
--   '123e4567-e89b-12d3-a456-426614174000'::UUID, -- fee_id
--   1500.00, -- amount
--   'COLLEGE_WIDE', -- scope_type
--   'Engineering', -- scope_college
--   NULL, -- scope_course
--   ARRAY['uuid1', 'uuid2']::UUID[] -- exempted students
-- );
  ...
  org_summary AS (
    ...
    FROM (
      SELECT 
        ...
        overall_rating,
        reaction_type,
        COUNT(*) OVER (PARTITION BY org_name, reaction_type) as reaction_count
      FROM filtered_feedback
    ) sub
    GROUP BY org_name
  )
...
--   p_org_name := 'Engineering College',
...
-- Example usage:
-- SELECT * FROM get_events_with_attendance_stats(
...
-- Example usage:
-- SELECT get_dashboard_stats('ADMIN');
-- SELECT get_dashboard_stats('COLLEGE_ORG', 'Engineering');
-- SELECT get_dashboard_stats('COURSE_ORG', 'Engineering', 'Computer Science');
...
-- Example usage:
-- SELECT * FROM cleanup_old_archived_records(); -- Use defaults
-- SELECT * FROM cleanup_old_archived_records(365, 500); -- 1 year, 500 batch size
...
CREATE OR REPLACE FUNCTION get_slow_queries(
  p_min_exec_time_ms NUMERIC DEFAULT 1000,
...
BEGIN
  ...
  SELECT 
    ...
    ROUND((100 * s.total_exec_time / SUM(s.total_exec_time) OVER ())::NUMERIC, 2) as percent_of_total,
    s.query::TEXT
  FROM pg_stat_statements s
  WHERE s.mean_exec_time >= p_min_exec_time_ms
...
COMMENT ON FUNCTION get_slow_queries IS 
  'Returns slowest queries from pg_stat_statements. Requires pg_stat_statements extension.';

-- Example usage (requires pg_stat_statements extension):
-- CREATE EXTENSION IF NOT EXISTS pg_stat_statements;
-- SELECT * FROM get_slow_queries(500, 10); -- Queries taking > 500ms
...
BEGIN
  ...
  RAISE NOTICE '  7. get_slow_queries()';
  RAISE NOTICE '';
  RAISE NOTICE 'Update your API endpoints to use these functions for better performance.';
END $$;
